...
var defaultOptions = options{
	...
	Development:     false,
	Format:          FormatJSON,
	LogFileLevel:    InfoLevel,
	LogConsoleLevel: InfoLevel,
	AddCaller:       false,
	CallerSkip:      1,
}
...
func InitLogger() {
	...
	var pointOptions = options{
		...
		LogDirs:         []string{config.LogDir},
		Format:          FormatJSON,
		LogFileLevel:    ParseLevel(config.LogFileLevel),
		LogConsoleLevel: ParseLevel(config.LogConsoleLevel),
		AddCaller:       true,
		CallerSkip:      1,
	}

	global = newLogger(pointOptions)
}
...
func newLogger(opts options) *Logger {
	...
	// add stdout log
	if opts.LogConsoleLevel >= _minLevel && opts.LogConsoleLevel <= _maxLevel {
		stdoutCore := zapcore.NewCore(
			encoder,
			zapcore.Lock(os.Stdout),
			zap.LevelEnablerFunc(opts.ZapConsoleLevelEnabled),
		)
	...
	if opts.Output != nil {
		outputCore := zapcore.NewCore(
			...
			zap.LevelEnablerFunc(opts.ZapFileLevelEnabled),
		)
	...
	for _, dir := range opts.LogDirs {
		...
		for _, level := range []zapcore.Level{
			...
			if opts.ZapFileLevelEnabled(level) {
				lvl := level
