...
func (c *Client) RawSetFloat(key string, value float64) (float64, error) {
	err := c.rdb.HSet(c.rdb.Context(), key, "value", value).Err()
	return value, err
}

func (c *Client) IncreaseBy(key string, value float64) (float64, error) {
	return c.rdb.HIncrByFloat(c.rdb.Context(), key, "value", value).Result()
}

func (c *Client) DecreaseBy(key string, value float64) (float64, error) {
	return c.rdb.HIncrByFloat(c.rdb.Context(), key, "value", -value).Result()
}
...
func (c *Client) LockForUpdateAndWait(key string, owner string, timeInterval time.Duration) error {
	lockUtil := math.Pow10(10)
	for {
		res, err := callLua(c.rdb.Context(), c.rdb, ` -- luaLock
	local lu = redis.call('HGET', KEYS[1], 'lockUtil')
